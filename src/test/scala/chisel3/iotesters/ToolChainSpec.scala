...
class ToolChainSpec extends FreeSpec with Matchers {
  ...
  for ( ebct <- editableBuildCommandTests) {
    ...
    s"Ability to edit vcs command line - $builderName" - {
      ...
      val expectedMultipleEditVCSCommand = """cd dir && vcs -full64 -loud -timescale=1ns/1ps -debug_access+all -Mdir=top.csrc +vcs+lic+wait +vcs+initreg+random +define+CLOCK_PERIOD=1 -P vpi.tab -cpp g++ -O2 -LDFLAGS -lstdc++ -CFLAGS "-I$VCS_HOME/include -I$dir -fPIC -std=c++11" -o top top.v harness.v vpi.cpp"""
      ...
      "can be done from multiple edits in a file" in {
        val edits = Seq(
          s"""verbose""",
          s"""s/-quiet /-loud /""",
          s"""s/\\+v2k \\+vpi //""",
