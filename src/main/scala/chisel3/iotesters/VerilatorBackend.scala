...
object VerilatorCppHarnessGenerator {
  def codeGen(dut: RawModule, state: CircuitState, vcdFilePath: String): String = {
    val codeBuffer = new StringBuilder

    val resetPathOption = dut.getPorts.find(p => p.id.instanceName == "reset")
...
""")
    ...
    //TODO: (chick) is this equivalent to the former dut.reset.getPathName
    //TODO:         that was available when dut was a MultiIOModule
    resetPathOption.foreach { resetPath =>
      codeBuffer.append(s"""        sim_data.signal_map["${resetPath}"] = 0;""")
    }
    codeBuffer.append(s"""}
#if VM_TRACE
...
private[iotesters] object setupVerilatorBackend {
  def apply[T <: RawModule](dutGen: () => T, annotationSeq: AnnotationSeq): (T, Backend) = {
    ...
    val annotationsWithRunFirrtlCompiler = annotationSeq :+ NoRunFirrtlAnnotation

    val targetDirName = annotationSeq.collectFirst { case TargetDirAnnotation(d) => d }.get
    val dir = new File(targetDirName)

    // Generate CHIRRTL
    chisel3.Driver.execute(Array.empty[String], annotationsWithRunFirrtlCompiler) match {
      case ChiselExecutionSuccess(Some(circuit), emitted, _) =>
         ...
         */
        val annotations = annotationsWithRunFirrtlCompiler ++ List(BlackBoxTargetDirAnno(targetDirName), FirrtlSourceAnnotation(emitted))

        val firrtlOptions = view[FirrtlExecutionOptions](annotations).get

        val transforms = annotationsWithRunFirrtlCompiler.collect { case RunFirrtlTransformAnnotation(t) => t }

        copyVerilatorHeaderFiles(targetDirName)
        ...
        val testCmd = annotationSeq.collectFirst { case TesterTestCommand(command) => command }.getOrElse(Seq.empty)

        val command = if(testCmd.nonEmpty) {
          testCmd
        }
        else {
...
private[iotesters] class VerilatorBackend(dut: RawModule,
                                          cmd: Seq[String],
